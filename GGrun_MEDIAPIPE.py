  #!/usr/bin/env python3
"""
=== MEDIAPIPE-ONLY VERSION ===
Optimized gesture control system using only MediaPipe for hand tracking.
No PyTorch/CUDA dependencies required - uses MediaPipe's built-in GPU acceleration.
...
os.environ['MEDIAPIPE_DISABLE_GPU'] = '0'  # Enable MediaPipe GPU acceleration 启用MediaPipe GPU加速

# MediaPipe optimization settings
os.environ['GLOG_minloglevel'] = '2'  # Reduce MediaPipe logging
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # Disable oneDNN optimizations for stability
...
# MediaPipe GPU加速支持 - 无需PyTorch
GPU_AVAILABLE = True  # MediaPipe handles GPU acceleration internally
print("Using MediaPipe with built-in GPU acceleration support")
...
class PerformanceMonitor:
    """
    Simplified Performance Monitor for MediaPipe-only version.
    Provides essential system metrics without PyTorch/CUDA dependencies.
    """
    ...
    def _monitor_hardware_loop(self):
        while self.monitoring_active:
            try:
                ...
                # GPU monitoring simplified for MediaPipe
                if GPU_AVAILABLE:
                    try:
                        # Use system-level GPU monitoring instead of CUDA-specific
                        import pynvml
                        pynvml.nvmlInit()
                        ...
                        self.gpu_memory_percent = (memory_info.used / memory_info.total) * 100
                    except Exception:
                        self.gpu_utilization = 0.0
                        self.gpu_temperature = 0.0
                        self.gpu_memory_percent = 0.0
    ...
    def _create_default_config(self) -> Dict[str, Any]:
        return {
            ...
            "ai": {
                ...
                "mediapipe_gpu": True,              # MediaPipe GPU acceleration
                "use_gpu": True,                    # Enable GPU acceleration with MediaPipe
...
class ComprehensiveLoggingManager:
    ...
    def log_system_information(self):
        ...
        # GPU information
        if GPU_AVAILABLE:
            self.logger.info("GPU Acceleration: Available (MediaPipe built-in)")
            try:
                import pynvml
                pynvml.nvmlInit()
                handle = pynvml.nvmlDeviceGetHandleByIndex(0)
                ...
                self.logger.info(f"GPU Memory: {memory_info.total / (1024**3):.1f} GB")
            except Exception:
                self.logger.info("GPU: Detection failed")
        else:
...
def main():
    ...
    if GPU_AVAILABLE:
        try:
            ...
            print(f"  GPU Memory: {memory_info.total / (1024**3):.1f} GB")
        except Exception:
            print("  GPU: MediaPipe GPU support available")
    else:
